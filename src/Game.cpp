...
bool Game::init(const wchar_t *title, int width, int height, bool fullscreen) {
  ...
  camera = m_pSceneManager->addCameraSceneNode(0, vector3df(60, 40, -40),
                                               vector3df(0, 5, 0));
  cameraPos = camera->getPosition();

  irr::gui::IGUISkin *skin = m_pGuiEnvironment->getSkin();
  irr::gui::IGUIFont *font =
      m_pGuiEnvironment->getFont("../assets/bigfont.png");
  ...
  fpsGui =
      m_pGuiEnvironment->addStaticText(L"FF", rect<s32>(10, 0, 100, 30), true);
...
void Game::clean() { m_pDevice->drop(); }

bool Game::isRunning() { return m_pDevice->run(); }
